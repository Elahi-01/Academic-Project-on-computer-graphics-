...
# -------------------------------------------------------------
# PyInstaller/EXE helper
# This makes extra files like boom.wav work both in normal Python
# and after converting the game into a Windows .exe file.
# -------------------------------------------------------------
...
moon_x = screen_width - 100  
moon_y = 100   
moon_radius = 50  


try:
    collision_sound = pygame.mixer.Sound(resource_path("boom.wav"))
except pygame.error:
    # If boom.wav is missing, the game will still run without sound.
    collision_sound = SilentSound()
...
def save_score(score, player_id, player_name):
    with open(writable_path("scores.txt"), "a") as file:
...
def score_display():
    ...
    try:
        with open(writable_path("scores.txt"), "r") as file:
